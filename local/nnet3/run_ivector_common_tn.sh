...
stage=0
nj=9
train_set=""   # you might set this to e.g. train.
test_sets=""
...
if [ $stage -le 3 ]; then
  ...
  steps/align_fmllr.sh --nj $nj  --cmd "$train_cmd"  --beam 100 --retry_beam 1200\
...
if [ $stage -le 6 ]; then
  ...
  steps/online/nnet2/train_diag_ubm.sh --cmd "$train_cmd" --nj 9 \
    --num-frames 700000 \
...
if [ $stage -le 7 ]; then
  ...
  diag_ubm=/media/storage0/hnaouara/ASR_MODELS_KALDI/vosk-model-ar-mgb2-0.4/am/ivector_extractor/
  steps/online/nnet2/train_ivector_extractor.sh --cmd "$train_cmd" \
    ...
    data/${train_set}_sp_hires $diag_ubm exp/nnet3${nnet3_affix}/extractor || exit 1;
